...
# RANOVA本体：プロセス全体の制御を行う関数
ranova <- function(dataset, design, ..., long = FALSE, type2 = FALSE, nopost = FALSE, tech = FALSE, data.frame = FALSE, copy = FALSE, 
	...
	info2 <- paste0("This output was generated by ranova 4.8.9 under ", strsplit(R.version$version.string, " \\(")[[1]][1], ".")# バージョン情報など
...
table.out <- function(dattable, titleinfo = NULL, subinfo1 = NULL, subinfo2 = NULL, use.header = NULL, smargin = nrow(dattable), 
	...
	dcnum <- ncol(dattable)# 列数
	drnum <- nrow(dattable)# 行数

	# ヘッダとテーブル本体の要素の表示マージンを決定する
	numlab <- sapply(1:dcnum,function(x) is.numeric(dattable[, x]))# 数値のみの列とそれ以外を区別
	...
	mblab <- colSums(bmat == wmat) != drnum# マルチバイト文字を含む列とそれ以外を区別
	...
	if(is.null(use.header)){# データフレームのヘッダをそのまま使う場合
		...
		hmmod <- tabhead != tabheadw# ヘッダがマルチバイト文字を含む列
	}else{# 指定された文字列を使う場合
	...
	coltype <- sapply(1:dcnum, function(x) is.factor(dattable[, x]) | is.character(dattable[, x]))# 各列がfactor型か文字型ならチェック
	...
	bmmat <- array(rep(bodymargin, each = drnum), c(drnum, dcnum))# マージンを行列形式に複製

	if(any(hmmod, na.rm = TRUE)) headmargin[hmmod] <- paste0("%", tabline[hmmod] + tabhead[hmmod] - tabheadw[hmmod], "s")
	if(any(mblab, na.rm = TRUE)){# マルチバイト文字を含む列がある場合
	...
	for (i in 1:drnum){# 行ごとの処理
		for(j in 1:dcnum){# 列ごとの処理
...
anovatan <- function(dataset, design, ..., tfact = NULL, long = FALSE, type2 = FALSE, nopost = FALSE, tech = FALSE, data.frame = FALSE, copy = FALSE, 
	...
	for(i in 1:length(subdat)){
		...
		ranova(dataset = subdat[[i]][, -(targetnm + 1)], design = subdesign, long = TRUE, type2 = type2, nopost = nopost, 
